  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res2 = await axios.get(
        `http://localhost:8080/api/user/valid/${values.username}/${values.validCode}`
      );
      if (res2.data === 200) {
  ...
  const handleSendMail = async (e) => {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8080/api/user/send/${values.username}`
      );
      setLoading(false);
      if (res) {
        ...
        toast.error("Email do not register please try again", {
          theme: "colored",
        });
      }
    } catch (err) {}
